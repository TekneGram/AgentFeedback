...
def compare_paragraphs(client: LlmClient, paragraph: str, max_tokens: int, temperature: float) -> str:
    ...
    if not s:
        return paragraph
    suggestion = client.chat(system=SYSTEM_COMPARE, user=s, max_tokens=max_tokens, temperature=temperature)
    return (suggestion or "").strip() or "Try adding a compare/contrast sentence to support your idea."

def filter_feedback(client: LlmClient, feedback: str, max_tokens: int, temperature: float) -> str:
    f = (feedback or "").strip()
    if not f:
        return feedback
    fb_new = client.chat(system=SYSTEM_FILTER, user=f, max_tokens=max_tokens, temperature=temperature)
